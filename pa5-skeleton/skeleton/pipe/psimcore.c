...
word_t cycles = 0;
/* How many instructions have passed through the WB stage? (WB: Write Back) */
...
static void update_state(bool_t update_mem, bool_t update_cc)
{
    ...
    */
	...
	if (mem_byte) {
		if ((wb_valM >> 7) & 1)
			set_reg_val(reg, wb_destM, -1);
		else
			set_reg_val(reg, wb_destM, 0);
		set_byte_val(reg, wb_destM * 8, wb_valM);
	}
	else set_reg_val(reg, wb_destM, wb_valM);
    }
...
void do_mem_stage()
{
    ...
    if (read) {
	dmem_error = dmem_error || (mem_byte? !get_byte_val(mem, mem_addr, &valm) : !get_word_val(mem, mem_addr, &valm));
	if (!dmem_error)
